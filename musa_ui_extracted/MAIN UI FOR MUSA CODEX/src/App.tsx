...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#EBF3FA] text-[#0E1E32] font-sans flex flex-col items-center justify-start antialiased selection:bg-[#D2E4F7] selection:text-[#0E1E32]">
      ...
      {/* Viewport Form Factor Switcher Bar for Reviewers */}
      <aside className="w-full bg-white/95 backdrop-blur-xs border-b border-[#D5E3F0] py-2 px-4 sm:px-6 flex items-center justify-between text-xs text-[#4D647E]">
        ...
        <div className="flex items-center gap-3">
          {/* Quick toggle between device frame & fluid layout */}
          <div className="flex items-center bg-[#EDF3FA] rounded-lg p-0.5 border border-[#D5E3F0]">
            <button
              onClick={() => setViewMode('mobile')}
              className={`flex items-center gap-1 px-2.5 py-1 rounded-md text-[11px] font-medium transition-all cursor-pointer ${
                viewMode === 'mobile'
                  ? 'bg-white text-[#0E1E32] shadow-2xs font-semibold'
                  : 'text-[#4D647E] hover:text-[#0E1E32]'
              }`}
      ...
      {/* Main App Container */}
      <div className={`w-full flex-1 flex flex-col justify-start items-center ${viewMode === 'mobile' ? 'py-4 sm:py-8' : 'py-0'}`}>
        <main
          className={`w-full flex flex-col justify-between bg-white transition-all ${
            viewMode === 'mobile'
              ? 'max-w-[430px] h-[844px] max-h-[90vh] rounded-2xl border border-[#CBD5E1] shadow-lg overflow-hidden'
              : 'max-w-3xl min-h-screen border-x border-[#CBD5E1] shadow-sm'
          }`}
        >
          {/* Active Screen View */}
