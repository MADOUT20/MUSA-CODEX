...
import { ShieldCheck, ArrowRight, Lock, PhoneCall, Copy } from 'lucide-react';
...
export const HomeScreen: React.FC<HomeScreenProps> = ({
  ...
  return (
    <div className="flex flex-col justify-between h-full px-5 sm:px-6 py-5 bg-[#FAFBFD] overflow-hidden select-none">
      ...
      {/* Main Center Content */}
      <div className="my-auto w-full max-w-sm mx-auto flex flex-col items-center gap-6">
        ...
        {/* Emergency Helpline Section (Moved from Support) */}
        <div className="w-full p-4 rounded-xl bg-[#FDF5F5] border border-[#F5D5D8] text-left shadow-sm">
          <div className="flex items-start gap-3">
            <div className="w-9 h-9 rounded-lg bg-[#8A2432] text-white flex items-center justify-center shrink-0 mt-0.5">
              <PhoneCall className="w-4 h-4" />
            </div>
            <div className="flex-1">
              <span className="text-[10px] font-bold uppercase tracking-wider text-[#8A2432] block mb-1">
                24/7 Campus Crisis Line
              </span>
              <h2 className="text-xs font-semibold text-[#0C2340] leading-tight">
                Immediate Security Intervention
              </h2>
              <p className="text-[11px] text-[#556980] mt-1 leading-relaxed">
                Direct dispatch for urgent campus security or physical safety concerns.
              </p>
              <div className="mt-2 flex items-center gap-2">
                <a
                  href="tel:8005557233"
                  className="px-3 py-1.5 bg-[#8A2432] hover:bg-[#731E2A] text-white rounded-lg text-[11px] font-semibold flex items-center gap-1.5 transition-colors cursor-pointer"
                >
                ...
                <button
                  ...
                  className="p-1.5 text-[11px] text-[#526B84] hover:text-[#0C2340] rounded-lg border border-[#D9E2EC] bg-white hover:bg-[#F8FAFC] transition-colors cursor-pointer"
                  title="Copy number"
                >
        ...
        <p className="text-center text-xs text-[#556980] max-w-xs leading-relaxed">
